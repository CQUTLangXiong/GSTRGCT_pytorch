...
from GSTRGCT.lib.utils import scaled_Laplacian, cheb_polynomial
...
if __name__ == "__main__":
    import numpy as np
    x = torch.rand(64, 307, 3,12).to(DEVICE)
    adj_mx = np.random.random((307,307))
    astgcn =  make_model(DEVICE, nb_block=2, in_channels=3, K=1, nb_chev_filter=4, nb_time_filter=4,
                         time_strides=1, adj_mx=adj_mx, num_for_predict=3, len_input=12, num_of_vertices=307)
    out = astgcn(x)
    print(out.shape)